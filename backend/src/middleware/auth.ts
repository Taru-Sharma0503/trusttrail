...
export const requireNgo = (req: Request, _res: Response, next: NextFunction) => { if (!req.auth || !NGO_ROLES.includes(req.auth.role)) return next(new ApiError(403, 'FORBIDDEN', 'NGO authorization is required')); next(); };
